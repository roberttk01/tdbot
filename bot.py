...
async def on_message(message):
    ...
    # /gptruth Command
        ...
        prompt = ("Generate a fun, insightful truth function for a game. The question can be deep, funny, serious, "
                  "riscque, provocative, et cetera. But ultimately, it needs to be original and fun.")
        try:
        ...
        if custom_id == "truth":
            ...
            try:
                prompt = ("Generate a fun, insightful truth question for a truth or dare game. "
                          ...
                          "but it must be original and engaging.")
                response = openai.ChatCompletion.create(
                    model="gpt-4o-mini",
                    messages=[{"role": "user", "content": prompt}]
                )
                ...
                await interaction.response.send_message(embed=embed, ephemeral=False, view=create_buttons())
